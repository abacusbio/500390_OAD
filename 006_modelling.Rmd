...
```{r read_RData, cache=TRUE, cache.lazy=FALSE, cache.extra=tools::md5sum("e:/500390_OAD/output/production_survival_noYoungAnimal_ebv_beforeParity5.RData")}
full_df <- readRDS(
  paste0(fileDir, "production_survival_noYoungAnimal_ebv_beforeParity5.RData"))
...
TAD herd size to 296. See `006_sumstat.rmd`
...
cat(trait, " ");print(Sys.time()-t)
      ...
      s <- summary(glm_s12_model)
      coefs <- data.frame(s$coefficients[grepl("EBV|new", rownames(s$coefficients)),
                                         ,drop = F])
     names(coefs)[4] <- "p-value"
     coefs$sign <- ifelse(coefs$`p-value`<0.001, "***", 
                     ifelse(coefs$`p-value`<0.01, "**",
...
cat("\n", paste0("s", i-1, i), "\n")
  df_model <- select(df_downsample, AnimalDurableCode, HerdDurableKey, 
                   ...
                   starts_with("EBV_")) %>% 
  filter(AgeParity==i & 
           !grepl(paste0("Transition_s"), new_milk_type)) %>% # only OAD and TAD
  distinct() %>% 
  mutate(HerdDurableKey = as.factor(HerdDurableKey), 
         DairyYear = as.factor(DairyYear),
         cg = as.factor(cg),
    ...
    if(length(l$coef) > 0) {
      write.csv(l$coef, paste0(outputDir, "coef_glm_x_s", i-1, i, ".csv"),
              quote = F)
    }
    if(length(l$anova) >0) {
    write.csv(l$anova, paste0(outputDir, "anova_glm_x_s", i-1, i, ".csv"),
              quote = F)
    }
} # end do loop
