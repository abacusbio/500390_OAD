...
group_by(df_downsample, new_milk_type, HerdDurableKey) %>% 
  summarise(count = n()) %>% 
  ungroup() %>% group_by(new_milk_type) %>% 
  tally() %>% 
  mutate(prop = n/sum(n)) %>% 
  knitr::kable(align = "l", 
...
group_by(df_downsample, new_milk_type, AgeParity, HerdDurableKey) %>% 
  summarise(count = n()) %>% 
  ungroup() %>% group_by(new_milk_type, AgeParity) %>% 
  tally() %>% ungroup() %>% 
...
```{r n_cows_by_age}
group_by(df_downsample, herd_milk_type, AgeParity, new_milk_type) %>% 
 # summarise(count = n()) %>% 
