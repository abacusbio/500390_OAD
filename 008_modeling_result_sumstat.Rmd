...
```{r simple_model_coef_paper, fig.show='asis', fig.height=16, fig.width=10}
...
test$survival <- toupper(test$survival)

# add line breaks
test$Trait_new <- gsub("_", "\n", test$Trait)
test$Trait_new <- gsub("([[:lower:]])([[:upper:]])", "\\1\n\\2", test$Trait_new) 
 ...
 # lapply(seq(essential_traits), function(i) {
   ...
   # color <- ifelse(i==length(essential_traits), "black", "white")
    ...
    geom_text(aes(y = std_coef, label = sign_x),
              position = position_dodge(.9), size = 2, # color = "red",
              vjust = 1
              ) +
    labs(# title = bv #ifelse(i==1, paste0(bv, " ", essential_traits[i]),
                        # essential_traits[i]),
         x = "Survival", y = "Standardised coefficient"
         ) +
    facet_grid(Trait_new~., scales = "free_y", 
               # labeller = label_wrap_gen(width = 1)
               ) +
    scale_fill_manual(values = c("grey", "grey35")) +
    theme_pubclean() +
    theme(text = element_text(size = 7), strip.text.y = element_text(size = 8),
          legend.position = "none")
    print(p)
    
    ggsave(paste0(outputDir, "Fig2.png"), p, 
           limitsize = FALSE, width = 148, height = 202, dpi = 300, units= "mm",
           device="png")
 # })
# })
# )
