...
98% cows have herd tests in Sept, Nov or Dec, hence the 3 months.
...
```{r herd size}
size <- full_df %>% group_by(DairyYear, herd_milk_type, HerdDurableKey) %>% 
  tally() %>% ungroup() %>% 
  mutate(size = n) %>% 
  group_by(herd_milk_type, DairyYear) %>% 
  summarise(count = n(), min = min(size), median = median(size), max = max(size), 
            mean = mean(size), SD = sd(size))
...
df <- select(full_df, AnimalDurableCode, matches("BV_")) %>% 
  distinct()
