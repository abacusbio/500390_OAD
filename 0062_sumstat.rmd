...
full_df <- readRDS(paste0(fileDir, "modeling/df_downsample.RData"))
```
...
p <- ggplot(df, aes(survival, prp, fill = new_milk_type)) +
  ...
  geom_text(aes(y = prp, label = sign), position = position_dodge(.8),
            vjust = -0.05) +
  theme_pubclean() +
...
```{r ebv, cache=TRUE}
sumstat_ebv <- lapply(grep("EBV_", names(full_df), value = T), function(i) {
  
  # table for plot
  sumstat_ebv <- select(full_df, ends_with(i), # starts_with("EBV_"),
  ...
  #          data = select(full_df, new_milk_type, matches(i)) %>% 
...
write.csv(ebv_table, "sumstat_ebv.csv", quote = F, row.names = F)
...
```{r ebv1, cache=TRUE}
sumstat_gbv <- lapply(grep("GBV_", names(full_df), value = T), function(i) {
  
  # table for plot
  sumstat_gbv <- select(full_df, ends_with(i), # starts_with("EBV_"),
                      new_milk_type) %>% #, new_milk_type) %>% 
    group_by(new_milk_type) %>% 
    summarise(across(everything(), 
                     list(mean = mean, median = median, SD = sd))) %>% 
...
write.csv(gbv_table, "sumstat_gbv.csv", quote = F, row.names = F)
...
i <- grep("EBV_", names(full_df), value = T)[1]
  p <- ggboxplot(filter(full_df, !is.na(new_milk_type)),
...
traits <- grep("BV_", names(full_df), value = T) %>% 
...
ps <- lapply(unique(for_plot$trait), function(i) {
  df <- filter(for_plot, trait == i) %>% 
    ...
    mutate(sign = ifelse(p_value < 0.001, "***",
           ...
           new_milk_type = 
             factor(new_milk_type, 
                    levels = c("TAD", setdiff(unique(df$new_milk_type), "TAD"))))
...
```{r ebv within sur and milk, results='asis', eval = F}
...
by_sur <- lapply(2:5, function(i) {
  ...
  by_trait <- lapply(grep("EBV_", names(full_df), value = T), function(j) {
    
    df <- filter(full_df, AgeParity==i) %>% 
...
ps <- lapply(grep("EBV_", unique(df$trait), value = T), function(i) {
  df <- filter(df, trait == i)
  mutate(df, new_milk_type = 
             factor(new_milk_type, 
                    levels = c("TAD", setdiff(unique(df$new_milk_type), "TAD")))) %>% 
  ggplot(aes(new_milk_type, value, fill = survived)) +
  geom_bar(stat = "identity", width = .8, color = "black", 
           position = position_dodge(.8)) +
  geom_text(aes(y = value, label = sign), position = position_dodge(.8),
            vjust = -0.05) +
  facet_grid(survival~.) +
  labs(title = i, x = "") +
  theme_pubclean() +
